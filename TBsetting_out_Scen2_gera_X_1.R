...
for(k in 1:3){
  if(k==1){
    load("/project/6003552/widloro/git/TBsim_binbin_case1Exp_Scen2_de_X.RData")
    ...
    for(w in 1:nsim){
      ...
      if(w %in% seq(50,nsim,len=20)){print(w);print(timestamp());save.image("/project/6003552/widloro/git/TBsim_binbin_case1_Scen2_gera_X.RData")}
    }
  }
  if(k==2){
    load("/project/6003552/widloro/git/TBsim_binbin_case2Exp_Scen2_de_X.RData")
    beta.ests[[k]] <- OddsRatio.ests[[k]] <- var.beta.ests[[k]] <-  matrix(NA,nsim,4)
    for(w in 1:nsim){
      data.3 <- list(N = m*nrep,M=m,I=index, 
                     Z = data[[k]][[w]]$Z, Y = data[[k]][[w]]$Y, 
      ...
      OddsRatio.ests[[k]][w,1] <- mean(exp(chain.beta1$beta))
      OddsRatio.ests[[k]][w,2] <- mean(exp(chain.beta2$beta))
      if(w %in% seq(50,nsim,len=20)){print(w);print(timestamp());save.image("/project/6003552/widloro/git/TBsim_binbin_case2_Scen2_gera_X.RData")}
    }
  }
  if(k==3){
    ...
    for(w in 1:nsim){
      ...
      OddsRatio.ests[[k]][w,1] <- mean(exp(chain.beta1$beta))
      OddsRatio.ests[[k]][w,2] <- mean(exp(chain.beta2$beta))
      if(w %in% seq(50,nsim,len=20)){print(w);print(timestamp());save.image("/project/6003552/widloro/git/TBsim_binbin_case3_Scen2_gera_X.RData")}
    }
  }
}
